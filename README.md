...
Cette application est un batch périodique permettant de chauffer le cache de l'application [BACON](https://bacon.abes.fr). Ce batch va successivement appeler toutes les URL de tous les packages KBART de BACON ceci aura pour effet de mettre en cache le contenu de ces KBART dans le système de cache de BACON et ainsi leur futur téléchargement sera ultra rapide car chaque génération de KBART ne provoquera plus de traitement parfois intensif au niveau de la base de donnée.
